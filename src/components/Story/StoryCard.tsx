...
export default function StoryCard({ story, UserImage, UserName, userId, isMe }: { story: StoryType, UserImage: string, UserName: string, userId: number | string, isMe?: boolean }) {
  const navigate = useNavigate();

  return (
    <div 
      onClick={() => navigate(`/story/${userId}`)}
      className="relative group w-36 h-60 min-w-36 rounded-2xl overflow-hidden shadow-lg cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-xl bg-gray-900 border border-gray-200 dark:border-gray-800 shrink-0"
    >
      <img 
        src={story.mediaUrl} 
        alt="story image" 
